...
/**
 * @title StHypeLoopLeg2Borrow
 * @notice Executes leg 2 of the stHYPE Loop: borrow HYPE from Felix with wstHYPE collateral
 *
 * To run on testnet:
 * forge script script/StHypeLoopingStrategy/StHypeLoopLeg2Borrow.s.sol:StHypeLoopLeg2BorrowScript --rpc-url $RPC_URL --broadcast --skip-simulation --legacy
 */
contract StHypeLoopLeg2BorrowScript is StHypeLoopBase {
    ...
    function run() external {
        console.log("Starting StHype Loop Strategy - Leg 2 (Borrow)");
        ...
        try manager.manageVaultWithMerkleVerification(
            ...
            console.logString("Borrowed HYPE succcessfully");
        } catch (bytes memory err) {
            console.logString("Borrowing failed:");
            logError(err);
